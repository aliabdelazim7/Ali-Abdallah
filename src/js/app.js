...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  localStorage.removeItem("ali_theme");

  // Show login success toast if navigated from login screen
  const loginToast = sessionStorage.getItem("ali_login_toast");
  if (loginToast) {
    if (loginToast === "demo") {
    ...
    sessionStorage.removeItem("ali_login_toast");
  }
...
function initRouter() {
  ...
  const handleRoute = () => {
    ...
    const sessionActive = localStorage.getItem("ali_session_active") === "true" || sessionStorage.getItem("ali_session_active") === "true";
    if (!sessionActive) {
...
function initGlobalEvents() {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", (e) => {
      ...
      const correctEmail = currentConfig.adminEmail || "admin@aliabdullah.com";
      const correctPass = currentConfig.adminPassword || "Ali2026_Secure";

      // Clear any previous session details from both storages first
      localStorage.removeItem("ali_session_active");
      localStorage.removeItem("ali_demo_mode");
      sessionStorage.removeItem("ali_session_active");
      sessionStorage.removeItem("ali_demo_mode");

      const rememberMe = document.getElementById("login-remember")?.checked;
      const storage = rememberMe ? localStorage : sessionStorage;

      if (email === "demo@aliabdullah.com" && pass === "demo") {
        storage.setItem("ali_session_active", "true");
        storage.setItem("ali_demo_mode", "true");
        sessionStorage.setItem("ali_login_toast", "demo");
        location.reload();
      } else if (email === correctEmail && pass === correctPass) {
        storage.setItem("ali_session_active", "true");
        storage.setItem("ali_demo_mode", "false");
        sessionStorage.setItem("ali_login_toast", "admin");
        location.reload();
      } else {
...
function loadSettingsFromConfig() {
  ...
  window.appState.settings = {
    businessName: config.businessName || "محلات علي عبدالله للأدوات الكهربائية",
    currency: config.currency || "EGP",
    address: config.address || "",
    phone: config.phone || "",
    adminEmail: config.adminEmail || "admin@aliabdullah.com",
    adminPassword: config.adminPassword || "Ali2026_Secure",
    settingsPassword: config.settingsPassword || "ali_settings"
  };
  ...
  const isDemo = localStorage.getItem("ali_demo_mode") === "true" || sessionStorage.getItem("ali_demo_mode") === "true";
...
window.initAuth = function() {
  ...
  const CREDENTIALS_VERSION = "v1_ali";
  if (localStorage.getItem("ali_credentials_ver") !== CREDENTIALS_VERSION) {
    localStorage.removeItem("ali_session_active");
    sessionStorage.removeItem("ali_session_active");
    localStorage.removeItem("ali_demo_mode");
    sessionStorage.removeItem("ali_demo_mode");
    ...
    const migrateSettings = (key) => {
      ...
      if (raw) {
        try {
          ...
          if (config.adminEmail === "admin@aliabdullah.com" || !config.adminEmail) {
            config.adminEmail = "admin@aliabdullah.com";
          }
          if (config.adminPassword === "admin" || !config.adminPassword) {
            config.adminPassword = "Ali2026_Secure";
          }
          localStorage.setItem(key, JSON.stringify(config));
        } catch (e) {}
      }
    };
    migrateSettings("ali_system_settings");
    migrateSettings("ali_demo_settings");
        ...
        try {
          ...
          if (db && Array.isArray(db.Settings)) {
            ...
            db.Settings.forEach(s => {
              if (s.Key === "Admin Email" && (s.Value === "admin@aliabdullah.com" || !s.Value)) {
                s.Value = "admin@aliabdullah.com";
                modified = true;
              }
              if (s.Key === "Admin Password" && (s.Value === "admin" || !s.Value)) {
                s.Value = "Ali2026_Secure";
                modified = true;
              }
            });
            if (modified) {
    ...
    migrateDb("ali_local_database");
    migrateDb("ali_demo_database");

    localStorage.setItem("ali_credentials_ver", CREDENTIALS_VERSION);
  }
