...
function initializeDatabase() {
  ...
  if (settingsSheet.getLastRow() <= 1) {
    const defaultSettings = [
      ["Business Name", "محلات علي عبدالله للأدوات الكهربائية"],
      ["Address", "Cairo, Egypt"],
      ["Phone Number", "+20 123 456 7890"],
      ["Currency", "EGP"],
      ["Admin Email", "admin@aliabdullah.com"],
      ["Admin Password", "Ali2026_Secure"],
      ["Settings Password", "ali_settings"],
      ["Initialized", "true"]
    ];
...
function authenticateToken(token) {
  ...
  const lastRow = sheet.getLastRow();
  if (lastRow <= 1) {
    return token === "Ali2026_Secure";
  }
  
  const keys = sheet.getRange(2, 1, lastRow - 1, 2).getValues();
  let correctPassword = "Ali2026_Secure"; // default fallback
  for (let i = 0; i < keys.length; i++) {
